        ...
        try:
            ...
            if period == "daily":
                signals_result = await db.execute(
                    select(SignalModel).filter(func.date(SignalModel.date) == date.today())
                )
                ...
                geo_row = geo_result.scalars().first()
                if geo_row:
    ...
    async with get_async_session() as db:
        try:
            ...
            existing_result = await db.execute(
                select(DailyReport).filter(DailyReport.date == today)
            )
            ...
            signals_result = await db.execute(
                select(SignalModel).filter(func.date(SignalModel.date) == today)
            )
            signals_today = signals_result.scalars().all()
            ...
            port_result = await db.execute(select(PortModel))
            portfolio_positions = port_result.scalars().all()
            if portfolio_positions:
                port_inst_ids = list({p.instrument_id for p in portfolio_positions if p.instrument_id})
                inst_result = await db.execute(
                    select(Instrument).filter(Instrument.id.in_(port_inst_ids))
                )
                port_insts = inst_result.scalars().all()
    ...
    async with get_async_session() as db:
        try:
            ...
            if inst_ids:
                ...
                for row in daily_result.scalars().all():
