import type { Bond } from "@/features/bonds/types/bond";
...
import type { ChartData, ChartRange } from "@/features/market/types/chart";
...
export async function getBonds(): Promise<Bond[]> {
  ...
  const res = await fetch("/api/proxy/bonds");
  if (!res.ok) throw new Error("Failed to fetch bonds");
  const data = await res.json();
  return (Array.isArray(data) ? data : []).map(mapBondResponse);
}
...
export async function getBondByTicker(ticker: string): Promise<Bond> {
  ...
  const res = await fetch(`/api/proxy/bonds/${ticker}`);
