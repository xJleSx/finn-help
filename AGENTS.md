# Output rules — экономия токенов (СОБЛЮДАТЬ ВСЕГДА)

- **НИКОГДА не выводи код в текстовых ответах.** Ни строки, ни diff, ни файл целиком.
- Инструменты `edit`/`write`/`bash` сами показывают результат в TUI. Не дублируй их вывод текстом.
- На каждый чих — максимум 1-3 предложения: что сделано, где, зачем.
- Для изменений существующих файлов используй ТОЛЬКО `edit` (он шлёт diff). `write` используй только для новых файлов.
